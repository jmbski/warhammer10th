...
import { ColumnDefinition, DataSheet, Keywords, ListItem, Profile, Weapon } from 'src/app/data/types';
...
export class DataCardComponent {
    ...
    public unit_abilities: any = {};
    public invulnerable_save: string = "";
    public composition: string = "";
    public points: number = 0;
    public wargear_abilities: any = {};
    public wargear_options: any = [];
    ...
    set dataSheet(input: DataSheet | undefined) {
        ...
        if (Object.keys(this.unit_abilities).length > 0) {
            this.unitAbilities = this.parseObjectToList(this.unit_abilities);
            this.hasUnitAbilities = true;
        }

        if (Object.keys(this.wargear_abilities).length > 0) {
            this.wargearAbilities = this.parseObjectToList(this.wargear_abilities);
            this.hasWargearAbilities = true;
        }

        if (Object.keys(this.composition).length > 0) {
            this.unitCompositionOptions = this.parseObjectToList(this.composition);
            this.hasComposition = true;
        }

        if (Object.keys(this.wargear_options).length > 0) {
            this.wargearOptions = this.parseObjectToList(this.wargear_options);
    ...
    public useStatBox(colField: string, value?: string) {
        ...
        else if (colField === 'weapon_range' && value && value.toLowerCase() === 'melee') {
            result = false;
        }
        return result;
    }
    ...
    public addSpaces(input: Weapon[]) {
        if (input && input.length > 0) {
            input.forEach((weapon: Weapon) => {
                if (weapon.core_rules != undefined && weapon.core_rules.length > 0) {
    ...
    public parseObjectToList(input: any) {
    ...
    public linkToDatasheet(unitName: string) {
        ...
        const datasheet: any = AllDataSheets.getValue().find(sheet => sheet.unit_name.toLowerCase() === unitName.toLowerCase());
        if (datasheet) {
