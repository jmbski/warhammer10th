...
import { Army, DataSheet, ListItem } from 'src/app/data/types';
    ...
    const subscription: Subscription = LoadingSubject.subscribe((value) => {
      if (!value) {
        ...
        if (lastDataSheetString) {
          const data: Partial<DataSheet> = JSON.parse(lastDataSheetString);
          if (data) {
  ...
  public filterSheetsByName(event: string) {
    if (this.dataSheets) {
      ...
      for (let i = 0; i < this.dataSheets.length; i++) {
        const datasheet: DataSheet = (this.dataSheets as DataSheet[])[i];
